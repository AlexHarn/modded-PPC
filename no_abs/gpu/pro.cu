...
__global__ void propagate(dats * ed, unsigned int num){
  ...
  // create array to log traveled distance in layers
  float layer_travel[MAXLYS];
  int last_layer = 0;
	...
	if(l<0){
	  ...
	  if(e.ka>0){
	    if(l==-128){
	      ...
	      do{ xi=1-l*e.ka*logf(xrnd(s)); } while (xi<-1);
	    }
	    float si=sqrtf(1-xi*xi); rotate(xi, si, n, s);
	  }
	}
	else{ // photon source not a flasher
	  if(l>0) l*=xrnd(s);
#ifdef LONG
  ...
  for(unsigned int i=idx; i<num; TOT==0 && (XINC)){
...
#ifdef HOLE
    {
#endif
      // layer stepping
      int i=__float2int_rn((z-e.hmin)*e.rdh); // current layer
      if(i<0) i=0; else if(i>=e.size) i=e.size-1;
      float h=e.hmin+i*e.dh; // middle of the layer
      h=n.z<0?h-e.hdh:h+e.hdh; // height to next layer

      float ais=(n.z*SCA-(h-z)*w->z[i].sca)*e.rdh;
      float aia=(n.z*TOT-(h-z)*w->z[i].abs)*e.rdh;

      // first layer travel distance
      float first_distance = abs(h - z)/abs(n.z);

      // fully traversed distance
      float full_distance = e.dh/abs(n.z);

      last_layer = i;
      if(n.z<0) // downgoing
          for(; last_layer>0 && ais<0 && aia<0; h-=e.dh, ais+=w->z[last_layer].sca, aia+=w->z[last_layer].abs)
          {
              --last_layer;
              // pretend that each layer is fully traversed for now, we'll correct later
              layer_travel[last_layer] += full_distance;
          }
      else // upgoing
          for(; last_layer<e.size-1 && ais>0 && aia>0; h+=e.dh, ais-=w->z[last_layer].sca, aia-=w->z[last_layer].abs)
          {
              ++last_layer;
              // pretend that each layer is fully traversed for now, we'll correct later
              layer_travel[last_layer] += full_distance;
      ...
      if(i==last_layer || fabsf(n.z)<XXX)
      {
          sca=SCA/w->z[last_layer].sca;
          tot=TOT/w->z[last_layer].abs;

          //cerr << "sca " << sca << "\n";
          // everything happening in this one layer, so just count the entire scattering distance
          layer_travel[last_layer] += SCA/w->z[last_layer].sca;
      }
      else
      {
          sca=(ais*e.dh/w->z[last_layer].sca+h-z)/n.z;
          tot=(aia*e.dh/w->z[last_layer].abs+h-z)/n.z;
          //cerr << "sca " << sca << "\n";

          // add first layer
          layer_travel[i] += first_distance;
          // correct the last layer
          layer_travel[last_layer] += sca - first_distance - abs(last_layer - i)*full_distance;
      }

      // get overburden for distance
      if(tot<sca) sca=tot, IFH(tot,TOT)=0; else IFH(tot=,TOT=nr*)(tot-sca)*w->z[last_layer].abs;
    }
	...
	if(D>=0){
	  ...
	  if(h2>=0 && h1<=sca*np){
		...
		if(h>0 && h<=del)
        {
            // subtract difference from last layer
            // TODO: do this in an exact way, not approximately
            layer_travel[last_layer] -= del - h;
            if (layer_travel[last_layer] < 0) // correct second to last layer
            {
                if(n.z<0) // downgoing
                    layer_travel[last_layer + 1] -= layer_travel[last_layer];
                else // upgoing
                    layer_travel[last_layer - 1] -= layer_travel[last_layer];
                layer_travel[last_layer] = 0;
            }
    ...
    { // advance
      ...
      if(r.w > 5000)
      {
          ...
          //{
              ////cerr << i << " " << layer_travel[i] << "\n";
              //s += layer_travel[i];
          //}
          //cerr << "total distance " << r.w / w->ocm << "\n";
          //cerr << "summed layer distances " << s << "\n\n";
      }
    }
    ...
    if(om!=-1){ // hit
      ...
      for (int i = 0; i < e.size; i++)
      {
          h.layer_travel[i] = layer_travel[i];
          //s += layer_travel[i];
      }
      //cerr << "total distance " << r.w/w->ocm << "\n";
      //cerr << "summed layer distances " << s << "\n\n";
      ...
      h.dth=acosf(min(max(dz/e.R, -1.f), 1.f)); h.dph=atan2f(dy, dx);
#endif
	...
	// no angular acceptance curve but a cut on the impact point as needed for SpiceHD
#ifndef PDIR
	sqd*=1-e.zR;
	const DOM & dom=oms[om];
	float dz=dom.r[2]-r.z+sqd*n.z;
