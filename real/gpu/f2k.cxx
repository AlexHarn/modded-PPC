...
struct mcid:pair<int,unsigned long long>{
  int frame;
  mcid() : frame(0){ first=-1, second=-1ULL; }
};
...
#else
  return
    ah.n!=bh.n ? (int)(ah.n-bh.n) :
    ah.i!=bh.i ? (int)(ah.i-bh.i) :
    ah.t!=bh.t ? sign(ah.t-bh.t) :
    sign(ah.z-bh.z);
#endif
}
...
void print(){
  ...
  for(unsigned int i=0; i<d.hidx; i++){
    ...
    if(n.hv>0 && xrnd()<f){
      ...
      // float *r = q.oms[h.i].r;
      float r[]={0, 0, 0};
      p.r[0]=r[0]-OMR*dts*dpc;
      p.r[1]=r[1]-OMR*dts*dps;
      p.r[2]=r[2]-OMR*dtc;
      p.r[3]=h.t;
      p.n[0]=pts*ppc;
      p.n[1]=pts*pps;
...
#ifdef CLST
void addp(float rx, float ry, float rz, float t, unsigned long long num, float dr, float beta){
  p.r.w=t; p.r.x=rx; p.r.y=ry; p.r.z=rz;
  p.l=dr;
  p.f=1.f;
  p.a=beta, p.b=0;
