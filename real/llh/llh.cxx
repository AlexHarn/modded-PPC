...
#define ANUM 11
  ...
  asens(){
	...
	if(dx.size()>0) cerr<<"Loaded "<<dx.size()<<" cable orientations"<<endl;
...
float spe(const pair<xppc::ihit, unsigned int>& dh){
  return spe(dh.second);
}
#endif
...
void cascade::runf(map< key, vector<hix> > & sim){
  const double ini=5, emax=100, bunch=2.5e9, ovr=xppc::xr*xppc::xr, width=fdur;
  ...
  if(oldf){
	...
	for(int j=0; j<jnum; j++){
	  xppc::flone(e*unfE[j]*bunch*(ang.flag?srep:1)/ovr);
	}
      }

#ifdef POUT
      if(pass && ang.flag){
	for(unsigned int n=0; n<ang.ds.size(); n++){
	  map< key, vector<hix> > sim;
	  for(xppc::outz::const_iterator i=xppc::hitz.begin(); i!=xppc::hitz.end(); ++i){
	    const xppc::ihit & h = i->first;
	    key om=make_pair(h.omkey.str, h.omkey.dom);
	    sim[om].push_back(hix(t+h.time+width*xrnd(), spe(*i, n), h.dir));
	  }
	  ...
	  for(map<key, dat>::iterator i=all.begin(); i!=all.end(); ++i){
	    const key & om=i->first;
	    dat & d=i->second;
	    if(d.in()) cout<<n<<" "<<om.first<<" "<<om.second<<" "<<d.llh(0, srep)/ndof<<endl;
	  }
	  end();
	}

	for(unsigned int n=0; n<ang.cs.size(); n++){
	  map< key, vector<hix> > sim;
	  for(xppc::outz::const_iterator i=xppc::hitz.begin(); i!=xppc::hitz.end(); ++i){
	    const xppc::ihit & h = i->first;
	    key om=make_pair(h.omkey.str, h.omkey.dom);
	    sim[om].push_back(hix(t+h.time+width*xrnd(), spe(*i, -1, n), h.dir));
	  }
