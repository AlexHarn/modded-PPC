...
echo "setting up DOM list"

for str in "${strings[@]}"; do
#for str in $strings; do
    if test $k = 0; then LYS=`seq 54 55`; m=-1
    else
...
#for str in $strings; do
	...
	fla=${str}_$dom
	dat=../dat/all/oux.$fla
	#dat=/data/user/mrongen/HoleIce/PPC/dima/exec/dima-new/MC_MC/dat/m085d2300m00/oux.$fla	

        if test -e $dat; then
            if ! test -d ../fit.$b-$n; then
		#num=`cat /data/user/mrongen/HoleIce/PPC/dima/exec/dima-new/MC_MC/dat/m085d2300m00/num.$fla`
		    ...
		    echo "time PPCTABLESDIR=$ice/$b FWID=$FWID SREP=$SREP DREP=$num FLSH=$fla FAIL=1 FAST=0 MLPD=0 CYLR=1 FLOR=0 FSEP=1 GSL_RNG_SEED=$RANDOM \$llh $m" > ../fit.$b-$n/run
		fi
	    fi
	    n=$[$n+1]
	fi
